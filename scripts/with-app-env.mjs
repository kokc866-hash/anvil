...
/** Put local npm bins first. Windows uses PATH or Path. */
export function withLocalBin(env) {
  const next = { ...env };
  const bins = binDir();
  const prev = next.PATH || next.Path || "";
  const value = prev.startsWith(bins) ? prev : `${bins}${delimiter}${prev}`;
  next.PATH = value;
...
/**
 * Resolve `vite` / other npm bins so Windows does not spawn ENOENT.
 * `.js` files run with this Node; `.cmd` shims use the shell.
 */
export function resolveCommand(command) {
  if (!command) return { file: command, extra: [], shell: false };
  const js = [".js", ".mjs", ".cjs"].includes(extname(command).toLowerCase());
  if (js) {
    const file = isAbsolute(command) ? command : join(projectRoot(), command);
    return { file: process.execPath, extra: [file], shell: false };
  }
  if (isAbsolute(command) || command.includes("/") || command.includes("\\")) {
    return { file: command, extra: [], shell: false };
  }
  const bins = binDir();
  if (process.platform === "win32") {
    const cmd = join(bins, `${command}.cmd`);
    if (existsSync(cmd)) return { file: cmd, extra: [], shell: true };
  }
  const local = join(bins, command);
  if (existsSync(local)) return { file: local, extra: [], shell: false };
  return { file: command, extra: [], shell: process.platform === "win32" };
}
...
function main(argv) {
  ...
  const child = spawn(resolved.file, [...resolved.extra, ...args], {
    stdio: "inherit",
    env,
