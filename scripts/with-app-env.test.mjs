...
test("resolveCommand runs local vite.js via this Node", () => {
  const r = resolveCommand("./node_modules/vite/bin/vite.js");
  assert.equal(r.file, process.execPath);
  assert.equal(r.shell, false);
  assert.match(r.extra[0], /vite\.js$/);
});

test("withLocalBin prepends node_modules/.bin", () => {
  const env = withLocalBin({ PATH: "/usr/bin" });
