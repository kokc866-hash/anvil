...
test("memory persistence recovers complete state beyond localStorage", async t => {
  ...
  const server = await createServer({ configFile: false, root: process.cwd(), resolve: { alias: { "@": path.resolve("src") } }, server: { middlewareMode: true, hmr: false, watch: null }, appType: "custom", plugins: [{ name: "archive-adapter", transform(code, id) {
  ...
  let flushPersistence = async () => {};
  t.after(async () => {
    try {
      await flushPersistence().catch(() => {}); await new Promise(resolve => setImmediate(resolve));
    } finally {
      await server.close();
  ...
  const persistence = await server.ssrLoadModule("/src/lib/persist-storage.ts");
  const { idePersistStorage } = persistence;
