  ...
  return <SettingsSection q={q}>
    <Vis q={q} label="Hilfe Help Support Fehlerbericht Diagnose Diagnostics Version Lizenz License Datenschutz Privacy Daten Herausgeber Publisher Veröffentlichung Release">
      ...
      <dl className="mt-3 grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 text-xs">
        ...
        <dt className="text-muted">Support</dt><dd>{productInfo.supportUrl ? <a href={productInfo.supportUrl} target="_blank" rel="noreferrer" className="underline">{label("Support öffnen", "Open support")}</a> : label("Ein Supportkontakt wird später ergänzt. Du kannst bereits einen Fehlerbericht vorbereiten und kopieren.", "Contact will be added later. Prepare and copy a report locally.")}</dd>
      ...
      <ul className="space-y-2 pl-4 text-xs leading-relaxed text-muted list-disc">
        <li>{label("Dateien werden im gewählten Projektordner gespeichert. Anvils Profil liegt standardmäßig im Unterordner data des Anvil-Verzeichnisses. Andere Speicherorte findest du unter „Speicher“.", "Files live in the selected project folder. Anvil's profile defaults to data beside Anvil; configured exceptions appear under Storage.")}</li>
        <li>{label("Modellanfragen enthalten den für die Aufgabe ausgewählten Kontext. Je nach Verbindung werden sie auf deinem Computer, auf einem Server im Netzwerk oder bei einem Anbieter verarbeitet. Externe CLI-Programme können eigene Datenordner verwenden.", "Model requests contain the context selected for the task and go to the chosen connection: this computer, LAN or provider. External CLIs may use their own data folders.")}</li>
        <li>{label("„Ausführen“ startet den Code deines Projekts. Externe Werkzeuge, MCP-Verbindungen und die Ausführung über einen Netzwerkserver können weitere Systeme ansprechen.", "Run executes project code. External tools, MCP connections and a selected network execution can contact other systems.")}</li>
      ...
      {report ? <div className="mt-3">
        <label className="text-xs text-muted">{label("Vollständige Vorschau", "Complete preview")}<textarea aria-label={label("Vorschau des Fehlerberichts", "Problem report preview")} readOnly value={report} rows={16} className="mt-1 w-full rounded border border-border bg-bg p-2 font-mono text-xs text-fg" /></label>
        ...
        {copyError ? <p role="status" className="mt-1 text-xs text-muted">{label("Der Bericht konnte nicht kopiert werden. Markiere den Text in der Vorschau und kopiere ihn manuell.", "Clipboard unavailable. Select and copy the preview text manually.")}</p> : null}
      </div> : null}
    </Vis>
  </SettingsSection>;
}
