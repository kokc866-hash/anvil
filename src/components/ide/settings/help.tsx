...
export function HelpSettings() {
  ...
  return <section aria-label={text("Erklärhilfen und Tour", "Explanations and tour")} className="my-4 rounded-md border border-border p-3 text-sm">
    ...
    <p className="mt-1 text-xs leading-relaxed text-muted">{text("Lass dir die wichtigsten Bedienelemente erklären, wenn du den Mauszeiger darüber hältst oder sie mit der Tab-Taste auswählst. Die Tour führt dich Schritt für Schritt durch den Arbeitsablauf.", "Get explanations of key controls when you hover or focus them with Tab. The tour walks you through the workflow.")}</p>
    ...
    <p className="mt-2 text-xs text-muted">{text("Du kannst die Tour jederzeit überspringen. Sie sendet keine Daten und verändert keine Dateien.", "Skip at any time. The tour sends nothing and changes no files.")}</p>
  </section>;
}
