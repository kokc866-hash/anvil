...
export function ConnectionSummary({ compact = false }: { compact?: boolean }) {
  ...
  return <div className="mt-2 space-y-1 rounded-md border border-border p-3 text-[11px] text-muted" data-testid="connection-summary">
    ...
    <p>{en ? "Image inputs, streaming responses and thinking controls depend on the model and connection. Anvil offers the settings supported by the selected combination." : "Ob du Bilder senden, Antworten während der Erstellung lesen oder den Denkaufwand einstellen kannst, hängt vom Modell und der Verbindung ab. Anvil zeigt die jeweils verfügbaren Einstellungen an."}</p>
