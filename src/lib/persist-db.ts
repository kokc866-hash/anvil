...
export async function loadArchive(
  name: string,
): Promise<{ files: Record<string, string> | null; chat: ArchiveMessage[] | null; recovery: ArchiveSnapshot["recovery"] | null; state?: Record<string, unknown>; savedAt?: number }> {
  ...
  const metadata = result(kv.get(`${name}:state`));
  ...
  return {
    ...(saved?.state ? { state: saved.state as Record<string, unknown>, savedAt: Number(saved.savedAt) || 0 } : {}),
...
export type ArchiveSnapshot = { state?: Record<string, unknown>; savedAt?: number; files?: Record<string, string>; chat?: ArchiveMessage[]; recovery?: { pendingDiffs: unknown[]; editBases: Record<string, string | null> } };
...
export async function saveArchive(name: string, next: ArchiveSnapshot, previous: ArchiveSnapshot): Promise<void> {
  ...
  if (next.state && next.state !== previous.state) kv.put({ state: next.state, savedAt: next.savedAt }, `${name}:state`);
...
export async function removeArchive(name: string): Promise<void> {
  ...
  for (const suffix of [":files", ":files-present", ":chat-order", ":recovery", ":state"]) tx.objectStore("kv").delete(name + suffix);
  await done;
}
