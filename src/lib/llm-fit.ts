...
import { connectionMode, providerOf } from "./providers.ts";
...
export function isCloudOrAbo(provider: string): boolean {
  if (provider === "brain" || provider === "custom") return false;
...
export function fitCloudAbo(provider: string, model: string, mode?: "abo" | "key"): CloudAboFit | null {
  if (!isCloudOrAbo(provider)) return null;
  const hit = lookupKnown(model);
  const abo = connectionMode(provider, mode) === "abo";
  return {
    llmContextAuto: true,
