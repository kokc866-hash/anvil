...
function probe(lang: LangId, n: number, path: string, names: string[], withLocals: boolean): string {
  ...
  if (lang === "go") {
    const map = loc.map((id) => `"${id}": __anvilDbgValue(${id})`).join(", ");
  ...
  if (lang === "cpp") {
    const inner = loc.length
      ...
      : `""`;
    return `{ std::ostringstream __anvil_s; __anvil_s << ${JSON.stringify(`{"line":${n},"path":${JSON.stringify(path)},"locals":{`)} << ${inner} << "}}"; std::cerr << "${MARK}" << __anvil_s.str() << std::endl; }`;
  }
  if (lang === "c") {
  ...
  if (lang === "csharp") {
    return `System.Console.Error.WriteLine(${JSON.stringify(`${MARK}${JSON.stringify({line:n,path,locals:{}})}`)});`;
  }
  if (lang === "php") {
...
func __anvilDbgValue(value any) string { return fmt.Sprint(value) }
`,
    };
  }
  return null;
...
export function prepareDebugTrace(path: string, files: Record<string, string>, withLocals = true) {
  const lang = langFromPath(path);
  ...
  if (extra) {
    ...
    extra.name = parent + extra.name;
    if (Object.hasOwn(files, extra.name)) throw new Error('Reservierter Trace-Helfername im Projekt: ' + extra.name);
  }
  const pack = extra
    ? [
  ...
  return { lang, entry: path, files: pack };
}

export function stackOf(ev: TraceEvent): Array<{path: string; line: number; fn: string}> {
