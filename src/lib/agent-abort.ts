...
export function explainAbort(err: unknown): string {
  ...
  if (/signal is aborted without reason/i.test(msg)) {
    return "Die Anfrage wurde abgebrochen. Mögliche Ursachen sind ein manueller Stopp, ein Zeitlimit oder eine unterbrochene Verbindung. Du kannst die Nachricht erneut senden.";
  }
...
export function explainLlmError(err: unknown): string {
  ...
  if (/credit balance is too low|insufficient.?credit|billing/i.test(msg)) {
    return "Das Guthaben beim Anbieter reicht nicht aus. Prüfe dort deine Zahlungs- und Tarifeinstellungen und sende die Nachricht anschließend erneut.";
  }
  if (/401|invalid.?api.?key|unauthorized|authentication/i.test(msg)) return "Der API-Schlüssel fehlt oder ist ungültig. Prüfe ihn in den Verbindungseinstellungen.";
  if (/429|rate.?limit/i.test(msg)) return "Der Anbieter erhält zu viele Anfragen. Warte einen Moment und versuche es erneut.";
    ...
    timer =
      ms > 0
        ? globalThis.setTimeout(() => {
            finish(() => reject(new AgentAbortError(`Seit ${Math.round(ms / 1000)} Sekunden ist keine Antwort eingegangen. Die Anfrage wurde abgebrochen; du kannst sie erneut senden.`)));
          }, ms)
        : undefined;
    p.then(
      (v) => finish(() => resolve(v)),
